...
class  HttpCore<T> : Observable<T> {
    ...
    val syncFuture: T?
        ...
        get() {
            ...
            val request = request
                ...
                t = response.body()?.let { ParseUtil.parserFactory.responseBodyParser(mClass).convert(it) } as T
    ...
    fun getAsSingle() : Single<T>{
        ...
        return subscription.doOnDispose {
            Log.e(HttpCore::class.java.simpleName, "Single dispose")
            call.cancel()
            Log.e(HttpCore::class.java.simpleName, "Single call.cancel() : "+call.isCanceled)        }
    }
